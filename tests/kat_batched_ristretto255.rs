...
#[tokio::test]
async fn read_kat_batched_token_ristretto255() {
    // Check own KAT vectors
    let list: Vec<BatchedTokenTestVector> = serde_json::from_str(
        include_str!("kat_vectors/batched_ristretto255_vectors_privacypass.json").trim(),
    )
    .unwrap();
    ...
    let list: Vec<BatchedTokenTestVector> = serde_json::from_str(
        include_str!("kat_vectors/batched_ristretto255_vectors_go.json").trim(),
    )
    .unwrap();
...
#[tokio::test]
async fn write_kat_batched_token_ristretto255() {
    let nr = 5u16;
    ...
    let mut file =
        File::create("tests/kat_vectors/batched_ristretto255_vectors_privacypass-new.json")
            .unwrap();
