use blind_rsa_signatures::reexports::crypto_bigint::BoxedUint;
...
fn hex_to_boxed_uint(h: &[u8]) -> BoxedUint {
    let bytes: Vec<u8> = h
        .chunks(2)
        .map(|x| u8::from_str_radix(std::str::from_utf8(x).unwrap(), 16).unwrap())
        .collect();
    let bits_precision = bytes.len() as u32 * 8;
    BoxedUint::from_be_slice(&bytes, bits_precision).unwrap()
}
...
async fn public_metadata_tokens() {
    ...
    let sk = SecretKey::new(
        RsaPrivateKey::from_components(
            hex_to_boxed_uint(N_ENC),
            hex_to_boxed_uint(E_ENC),
            hex_to_boxed_uint(D_ENC),
            vec![hex_to_boxed_uint(P_ENC), hex_to_boxed_uint(Q_ENC)],
        )
        .unwrap(),
    );

    let public_key = sk.public_key().unwrap();
    ...
    let token_key_id = public_key_to_truncated_token_key_id(&key_pair.pk).unwrap();
    ...
    let challenge_digest = token_challenge.digest().unwrap();
    let extension = Extension::new(ExtensionType(10), b"Hello world".to_vec()).unwrap();
